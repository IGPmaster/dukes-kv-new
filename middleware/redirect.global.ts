// middleware/redirect.global.ts
import { defineNuxtRouteMiddleware, navigateTo } from '#imports'
import { WHITELABEL_ID, lang } from '~/composables/globalData'

export default defineNuxtRouteMiddleware(async (to, from) => {
  // Only handle promotion pages without trailing slash
  if (to.path.startsWith('/promotion/') && !to.path.endsWith('/')) {
    try {
      const slug = to.params.slug as string;
      console.log('Checking promotion:', slug);
      
      // Try to fetch the promotion data first
      const response = await fetch(
        `https://casino-promotions-api.tech1960.workers.dev/promotion/${slug}?brandId=${WHITELABEL_ID}&lang=${lang.value || 'IE'}`
      );
      
      if (response.ok) {
        console.log('Promotion found, redirecting to:', to.path + '/');
        // Data exists, redirect to same URL with trailing slash
        return navigateTo(to.path + '/', { redirectCode: 301 })
      } else {
        console.log('Promotion not found:', await response.text());
      }
    } catch (error) {
      console.error('Error checking promotion:', error);
    }
  }
})